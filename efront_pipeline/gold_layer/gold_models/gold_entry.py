...
from funcs.utils import load_config_module, ensure_dict_key_prefix
...
if IS_DATABRICKS:
    # dbutils.widgets.text("table_parameters_module", "")
    # dbutils.widgets.text("view_parameters_module", "")
    # dbutils.widgets.text("definitions_module", "")

    table_param_module_name = dbutils.widgets.get("table_parameters_module").strip()
    views_param_module_name = dbutils.widgets.get("view_parameters_module").strip()
    definitions_module_name = dbutils.widgets.get("definitions_module").strip()
else:
    # Fallback for local script execution
    table_param_module_name = "gold_table_parameters" 
    views_param_module_name = "gold_view_parameters"
    definitions_module_name = "gold_layer_definitions"

TABLE_CONFIG = load_config_module(table_param_module_name, "tables_config")
VIEW_CONFIG = load_config_module(views_param_module_name, "views_config")
DEFINITIONS = load_config_module(definitions_module_name, "gold_layer_definitions")

...
gold_fundoperations_investor_country_share_monthly_sql_code = '''
WITH detail AS (
  SELECT
    Fund,
    Fund_Structure_Type,
  ...
  FROM {silver_prefix}fundoperations_by_investor_country_monthly
),
totals AS (
  SELECT
    Fund,
  ...
  FROM {gold_prefix}fundoperations_monthly_totals_by_fund
)
SELECT
  d.Fund,
...
ORDER BY d.Fund, d.Year_Month, d.Transaction_Currency, d.Investor_Country
'''
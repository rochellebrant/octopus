table_rules = {
    ...
    "investoraccount": {
        "future_Created_on": "Created_on > current_date()",
        "future_Modified_on": "Modified_on > current_date()",
        "empty_Country": "TRIM(Country) = '' AND Investor NOT IN (SELECT Fund FROM oegen_data_prod_source.src_blackrock_efront_xio.fund)",
        "empty_Currency": "TRIM(Currency) = ''",
        "empty_Fund": "TRIM(Fund) = '' AND Investor NOT IN (SELECT Fund FROM oegen_data_prod_source.src_blackrock_efront_xio.fund)",
        "empty_Fund_IQId": "TRIM(Fund_IQId) = '' AND Investor NOT IN (SELECT Fund FROM oegen_data_prod_source.src_blackrock_efront_xio.fund)",
        "empty_Investor": "TRIM(Investor) = ''",
        "empty_Investor_Account": "TRIM(Investor_Account) = '' AND Investor NOT IN (SELECT Fund FROM oegen_data_prod_source.src_blackrock_efront_xio.fund)",
        "empty_Investor_Type": "TRIM(Investor_Type) = ''",
        "empty_Investor_Account_IQId": "TRIM(Investor_Account_IQId) = '' AND Investor NOT IN (SELECT Fund FROM oegen_data_prod_source.src_blackrock_efront_xio.fund)",
    },
    "track_record_asset": {

...
# --- Resolve bundle-driven config from DLT pipeline configuration ---
BUNDLE_TARGET = spark.conf.get("spark.xio.bundle_target", "")
DEVELOPMENT_PREFIX = spark.conf.get("xio.development_prefix", "dev_")
DATA_CATALOG = spark.conf.get("xio.data_catalog", "oegen_data_prod_test")
LANDING_SCHEMA = spark.conf.get("xio.landing_schema", "efront_landing")
BRONZE_SCHEMA = spark.conf.get("xio.bronze_schema", "efront_bronze")
SILVER_SCHEMA = spark.conf.get("xio.silver_schema", "efront_silver")
GOLD_SCHEMA = spark.conf.get("xio.gold_schema", "efront_gold")
...
for table in TABLE_CONFIG.keys():
    ...
    # Independent of the flagging above: persists the {table}_dupcheck aggregate table and
    # fails the pipeline update via a DLT expectation when duplicates are present, so the
    # run is still visibly Failed even though duplicate rows have already been routed to
    # quarantine rather than blocking the clean table.

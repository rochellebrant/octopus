...
# Attach “official” Fund and Portfolio identities
    # Uses internal reference/mapping tables to translate eFront names into:
        # ShCDM Fund ID (the canonical fund identifier used across the business)
...
# Convert transaction amounts into GBP
    # Takes each transaction in the fund’s base currency and converts it to GBP using a rate from the fx_vs_gbp silver table.
    # Timing: the FX rate used is NOT the rate on the actual transaction_date. The join matches fx_vs_gbp.Ref_Date to
    # last_day(transaction_date) — i.e. the month-end of the month the transaction fell in. So a transaction on any day
    # in a given month is converted using that month's month-end FX rate, not a daily/spot rate.
    # If currency is already GBP, the rate is hardcoded to 1.0.
    # If currency is not GBP and no matching (Currency, month-end Ref_Date) row exists in fx_vs_gbp, the left join
    # produces a NULL rate, so amount_gbp is NULL for that row (there is no fallback to 1.0 for non-GBP currencies —
    # this should be sense-checked/monitored for gaps in fx_vs_gbp coverage).
silver_aum_capital_injections_and_disposals_transactions_view_sql_code = """
with transactions as (
  select
    transaction_id,
    investment_details AS ef_fund,
    investment_portfolio AS ef_portfolio,

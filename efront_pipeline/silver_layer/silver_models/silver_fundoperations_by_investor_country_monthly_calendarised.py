...
silver_fundoperations_by_investor_country_monthly_calendarised_sql_code = '''
WITH src AS (
  SELECT
    Fund,
    Fund_Structure_Type,
...
monthly AS (
  ...
  FROM src
  GROUP BY Fund, Fund_Structure_Type, Fund_Status, Transaction_Currency, Investor_Country, ym
),
...
filled AS (
  SELECT
    c.Fund, c.Fund_Structure_Type, c.Fund_Status, c.Transaction_Currency, c.Investor_Country, c.ym,
  ...
  LEFT JOIN monthly m
   ...
   AND m.Investor_Country = c.Investor_Country
   AND m.ym = c.ym
),
detail AS (
  SELECT
    ...
    SUM(Commitments)  OVER (
      PARTITION BY Fund, Transaction_Currency, Investor_Country
      ORDER BY ym
...
FROM detail d
ORDER BY
  Fund, Year_Month, Transaction_Currency, Investor_Country
'''
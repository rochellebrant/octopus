...
def ingest_silver_entry_with_tombstones(spark, table, input_prefix, output_prefix, dev_prefix):
    table_name = f"{output_prefix}{table}"
    tombstone_table_name = f"{dev_prefix}silver_model_tombstones_{table}"
    
    @dlt.table(
        name=table_name,
        comment=f"Initial temporary landing table for {table}, unioned with tombstones.",
        temporary=True 
    )
    def get_unioned_data():
        # 1. Read the active Silver Entry models (This now has the perfect schema!)
        normal_stream = spark.readStream.option("ignoreChanges", "true").table(f"{input_prefix}{table}")
        
        # Ensure is_deleted exists for normal records
        if "is_deleted" not in normal_stream.columns:
            normal_stream = normal_stream.withColumn("is_deleted", f.lit(False))

        # 2. Try reading the tombstone stream and union them
        try:
            tombstone_stream = spark.readStream.option("skipChangeCommits", "true").table(tombstone_table_name)
            
            # Strip out the shared Bronze metadata columns
            cols_to_drop = ["datalake_ingestion_timestamp", "table_id"]
            for c in cols_to_drop:
                if c in tombstone_stream.columns:
                    tombstone_stream = tombstone_stream.drop(c)
            
            # Loop through the golden schema and force the tombstone columns to match
            for col_name, golden_type in normal_stream.dtypes:
                if col_name in tombstone_stream.columns:
                    tombstone_stream = tombstone_stream.withColumn(col_name, f.col(col_name).cast(golden_type))
            
            final_stream = normal_stream.unionByName(tombstone_stream, allowMissingColumns=True)
        except Exception as e:
            print(f"⚠️ No tombstone table found for {table} at {tombstone_table_name}. Proceeding with active stream only.")
...
dlt.create_streaming_table(f"{TARGET_TBL_PREFIX}quarantine_union")

for table in TABLE_CONFIG.keys():
    ingest_silver_entry_with_tombstones(
        spark=spark,
        table=table,
        input_prefix=SOURCE_TBL_PREFIX,
        output_prefix=f"{TARGET_TBL_PREFIX}landing_",
        dev_prefix=DEVELOPMENT_PREFIX
    )
    ...
    generate_unioned_quarantine_table(
        ...
        output=f"{TARGET_TBL_PREFIX}quarantine_union",
    )

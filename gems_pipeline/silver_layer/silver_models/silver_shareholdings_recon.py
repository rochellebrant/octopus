silver_recon_shareholdings_sql_code = """
WITH gems_full_history AS (
    -- 1. Get the full history for GEMs directly from the silver model
    SELECT
        internal_child_company_id AS child_internal_id,
        internal_parent_company_id AS parent_internal_id,
    ...
    FROM {silver_prefix}shareholdings_fact
    WHERE internal_child_company_id IS NOT NULL 
      AND parent_company_id IS NOT NULL 
      -- BUG FIX: Removed 'AND ownership_percentage > 0' to keep end-dated events
),
cdm_full_history AS (
...
historical_comparison AS (
    ...
    SELECT
        ...
        NULLIF(v.slice_end, CAST('9999-12-31' AS DATE)) AS active_to_date, 
        g.gems_pct,
        g.transaction_type AS gems_transaction_type, 
        c.cdm_pct
...
company_invp_fund_mappings AS (
    ...
    SELECT DISTINCT
        ...
        cfvp.investment_portfolio_id,
        invp.investment_portfolio_name,
        invp.invp_fund_core_id AS fund_core_id,
...
gems_entity_lookup AS (
    -- 7. Safely deduplicate the GEMs entities by internal_id
...
SELECT
    ...
    CAST(hc.active_from_date AS DATE) AS comparison_slice_start,
    CAST(hc.active_to_date AS DATE) AS comparison_slice_end,

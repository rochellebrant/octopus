...
def calculate_relationships(df, child_id_column, parent_id_column, max_level=15, min_pct=1e-6, max_paths_per_source=50000, all_results=False):
    """
    ...
    df[child_id_column] = df[child_id_column].astype(str)
    df[parent_id_column] = df[parent_id_column].astype(str)
    ...
    df["rel_id"] = df[parent_id_column] + "." + df[child_id_column]

    start_events = (
        df[df["ownership_percentage"] > 0]
        .rename(columns={"active_from_date": "event_date"})
        .assign(_kind="start")[
            ["event_date", "rel_id", parent_id_column, child_id_column, "ownership_percentage", "transaction_type", "_kind"]
        ]
    )
    ...
    for date in sorted(events["event_date"].unique()):
        ...
        changed_edges = list(zip(todays[parent_id_column], todays[child_id_column]))
        affected_nodes = set()
...
comp_ownr_df = (
    shareholdings
    .dropna(subset=["internal_parent_company_id", "internal_child_company_id"])
...
print(f"Processing {comp_ownr_df.count()} records...")

new_rels_pd, events = calculate_relationships(
    df=comp_ownr_df.toPandas(),
    child_id_column="internal_child_company_id",
    parent_id_column="internal_parent_company_id",
    max_level=15
)
...
if save_mode == "append":
    ...
    merge_into_delta(spark,
                     ...
                     delete_mode="none",       # Set to "none" because we want to preserve historical graph states
                     period_column=None,       # Not required since delete_mode is "none"
                     include_period_in_keys=False
                    )
else:

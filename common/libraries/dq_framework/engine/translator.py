...
class DQTranslator:
    def __init__(self, ws_client, base_path):
        ...
        self._ai_lock = (
            threading.Lock()
        )  # ensures AI generation is sequential so doesn't error
    ...
    def translate(self, check_item, table_cfg, config_rel_path):
        ...
        if check_item is None or not isinstance(check_item, dict):
            print(
                f"⚠️ SKIPPING: Invalid check item (None or non-dict) in config for {table}."
            )
            return []
        ...
        try:
            ...
            if ai_prompt:
                with self._ai_lock:
                    ai_output = self.generator.generate_dq_rules_ai_assisted(
                        [ai_prompt]
                    )
                    for r in ai_output:
            ...
            # Priority 2/3: Template logic
                ...
                if sidecar_fn:
                    try:
                        rules_found = sidecar_fn(
                            check_item.get("params", {}), table_cfg
                        )
                    except Exception as e:
                        raise RuntimeError(
                            f"Sidecar function '{template}' failed for table {table}: {e}"
                        )
                elif template in GLOBAL_REGISTRY:
                    try:
                        rules_found = GLOBAL_REGISTRY[template](
                            check_item.get("params", {}), table_cfg
                        )
                    except Exception as e:
                        raise RuntimeError(
                            f"Global Registry template '{template}' failed for table {table}: {e}"
                        )
                else:
                    raise ValueError(
                        f"Template '{template}' not found in Registry or Sidecar for table {table}."
                    )

            else:
                raise ValueError(
                    f"Check item in {table} missing both 'template' and 'ai_prompt'."
                )

        except Exception as e:
            raise RuntimeError(
                f"Translation Error for rule '{rule_name}' on table '{table}': {e}"
            )
    ...
    def _get_sidecar_fn(self, table_name, config_rel_path, fn_name):
        ...
        py_path = os.path.join(
            self.base_path, "configs", config_rel_path, f"{table_name}.py"
        )
        if os.path.exists(py_path):
            try:
                ...
                raise SyntaxError(
                    f"Critical error loading sidecar file '{py_path}': {e}"
                )

...
class DQRunner:
    def __init__(self, spark: SparkSession, runtime: Dict, base_path: str):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to initialize Workspace Client or DQEngine: {e}"
            )
        self.translator = DQTranslator(self.ws, base_path)
        self._ai_lock = (
            threading.Lock()
        )  # thread Lock for AI generation to prevent RateLimitErrors
    ...
    def run_for_table(self, config_rel_path: str, table_name: str):
        ...
        for check_item in full_cfg["merged_checks"]:
            try:
                if "ai_prompt" in check_item:
                    with self._ai_lock:
                        translated_list = self.translator.translate(
                            check_item, table_cfg, config_rel_path
                        )
                else:
                    translated_list = self.translator.translate(
                        check_item, table_cfg, config_rel_path
                ...
                rule_desc = (
                    check_item.get("name")
                    or check_item.get("template")
                    or "Unknown Rule"
                )
                raise RuntimeError(
                    f"Rule Translation failed for {table_name} [{rule_desc}]: {e}"
                )

        try:
            df = self.spark.read.table(source_fqtn)
            row_filter = table_cfg.get("filter")
            if row_filter:
                df = df.filter(row_filter)
            total_count = df.count()
        except Exception as e:
            raise RuntimeError(
                f"Data Access Error: Could not read table {source_fqtn}. Check permissions: {e}"
            )

        try:
            valid_df, invalid_df = self.dq_engine.apply_checks_by_metadata_and_split(
                df, error_checks
            )
            _, warning_df = self.dq_engine.apply_checks_by_metadata_and_split(
                df, warn_checks
            )
        except Exception as e:
            raise RuntimeError(
                f"DQ Execution failed on Spark for table {table_name}: {e}"
            )
    ...
    def _process_results(
        ...
        try:
            ...
            if invalid_df and warning_df:
                unified_df = invalid_df.unionByName(
                    warning_df, allowMissingColumns=True
                )
            else:
            ...
            if unified_df:
                ...
                exploded_df = unified_df.withColumn(
                    "failed_check", F.explode(combined_issues)
                )

                # 3. Extract the metadata from the exploded JSON object
                final_issues_df = (
                    exploded_df.withColumn("_check_name", F.col("failed_check.name"))
                    ...
                    .withColumn(
                        ...
                        F.coalesce(
                            ...
                            F.col("failed_check.user_metadata.check_columns").cast(
                                "string"
                            ),
                        ),
                    )
            ...
            raise RuntimeError(
                f"Result Storage Error for {table_name}: Failed writing to {dq_db}. Error: {e}"
            )
        ...
        if total_count > 0:
            success_rate_float = round(
                ((total_count - invalid_row_count) / total_count) * 100, 2
            )
        ...
        return {
            "emoji": (
                ...
                else ("⚠️" if warning_row_count > 0 else "✅")
            ),
            ...
            "status": (
                "Issues Found"
                if invalid_row_count > 0
                else ("Warning" if warning_row_count > 0 else "Success")
            ),

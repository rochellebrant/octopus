...
def apply_comments_from_definitions(
    ...
    existing_table_comment = None
    existing_column_comments: Dict[str, Optional[str]] = {}
    if skip_if_unchanged and not dry_run:
        existing_table_comment = get_existing_table_comment(
            spark, catalog, database, table
        )
        existing_column_comments = get_existing_column_comments(
            spark, catalog, database, table
        )
    ...
    if table_comment:
        new_table_comment = str(table_comment).strip()
        if (
            skip_if_unchanged
            and not dry_run
            and existing_table_comment == new_table_comment
        ):
            info(f"Table comment already up to date on {full_table_name}, skipping")
        else:
            sql_stmt = f"COMMENT ON TABLE {full_table_name} IS '{sql_escape(new_table_comment)}'"
            _exec_sql(spark, sql_stmt, dry_run)
            if not dry_run:
                info(f"Applied table comment on {full_table_name}")
    else:
        warn(
            f"No table comment found for {table_key} under ['table']['{comment_field}']"
        )
        result_row.mark_warning()
    ...
    undefined_in_def, defined_but_missing = diff_columns(
        defined_col_names, actual_col_names
    )
    ...
    for col in sorted(actual_col_names & defined_col_names):
        ...
        if col_comment:
            new_col_comment = (
                f"{str(col_long_name).strip()}. {str(col_comment).strip()}"
                f" \nUNIT: {str(col_unit).strip()}"
            )
            if (
                skip_if_unchanged
                and not dry_run
                and existing_column_comments.get(col) == new_col_comment
            ):
                skipped_unchanged += 1
                continue

            sql_stmt = (
                f"COMMENT ON COLUMN {full_table_name}.{quote_identifier(col)} IS "
                f"'{sql_escape(new_col_comment)}'"
            )
            _exec_sql(spark, sql_stmt, dry_run)
            if not dry_run:
    ...
    if skipped_unchanged:
        info(
            f"Skipped {skipped_unchanged} column comment(s) already up to date on {full_table_name}"
        )
...
def apply_comments_for_all_tables(
    ...
    info(
        f"Run summary: ok={len(summary.ok)} skipped={len(summary.skipped)} errors={len(summary.errors)}"
    )
    ...
    tables_with_col_warnings = [
        r for r in summary.ok if r.undefined_columns or r.defined_but_missing
    ]

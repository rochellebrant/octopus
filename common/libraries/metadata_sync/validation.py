...
def quote_identifier(name: str) -> str:
    """Backtick-quote a column/identifier so names like '2000' (all-digit) parse as identifiers, not literals."""
...
def validate_column_definitions_shape(column_definitions: Dict[str, Any]) -> None:
    ...
    for table_key, table_def in column_definitions.items():
        ...
        if not isinstance(cols, dict):
            raise TypeError(
                f"column_definitions['{table_key}']['columns'] must be a dict"
            )

        for col_name, col_def in cols.items():
            if not isinstance(col_def, dict):
                raise TypeError(
                    f"column_definitions['{table_key}']['columns']['{col_name}'] must be a dict"
                )
...
def list_tables_in_schema(spark, catalog: str, database: str) -> Set[str]:
    rows = (
        spark.sql(f"SHOW TABLES IN {catalog}.{database}").select("tableName").collect()
    )
...
def preflight(
    ...
    try:
        ...
        raise TableCommentsError(
            f"Preflight failed: cannot access schema {catalog}.{database}: {e}"
        )
...
def get_existing_table_comment(
    spark, catalog: str, database: str, table: str
) -> Optional[str]:
    rows = spark.sql(
        f"SELECT comment FROM {catalog}.information_schema.tables "
        f"WHERE table_schema = '{sql_escape(database)}' AND table_name = '{sql_escape(table)}'"
    ).collect()
    if not rows:
        return None
...
def get_existing_column_comments(
    spark, catalog: str, database: str, table: str

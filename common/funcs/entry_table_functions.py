        ...
        if (
            ...
            try:
                ...
                if len(attributes) == 1:
                    ...
                    print(
                        f"Stored {attr_name} from {module_name} in models_dict[{module_name}]."
                    )
                elif len(attributes) > 1:
                    print(
                        f"Warning: Multiple attributes found in {module_name}, using the first one."
                    )
                ...
                print(
                    f"Error importing module {module_name} from filepath {path_to_models}: {e}"
                )
...
def init_entry_table_common(
    ...
    try:
        ...
        raise Exception(
            f"\t> ⚠️ Failed to grant permission on {table_name}: {e}"
        ) from e
...
def check_if_table_is_empty(spark, target_tbl_prefix: str, table: str, models_config):
    ...
    if not timestamp_col:
        raise KeyError(
            f">>> ⚠️ 'load_date_column' parameter missing for table '{table}'."
        )
    ...
    try:
        ...
        print(
            f">>> ⚠️ Error in check_if_table_is_empty() for table {table_name}: {str(e)}"
        )
        raise e


def _drop_and_recreate_table(spark, table_name: str, query: str) -> None:
...
def load_entry_tables(
    ...
    table_type = models_config[table].get("table_type", "merge").lower()

    if table_type in ["replace", "overwrite"]:
        print(f"\t📤 Overwriting all records safely in {table_name}")
        # overwriteSchema=true merges the incoming schema onto the existing table's
        # metadata rather than replacing it outright, which can leave stale column
        # entries behind (e.g. a differently-bounded PIVOT) and raise
        # DELTA_DUPLICATE_COLUMNS_FOUND.DATA. A "replace" table_type means the schema
        # should be replaced too, so drop the table first and recreate it fresh from
        # the DataFrame, then reapply the permissions init_entry_table_common granted.
        # Note: dropping the table also wipes any COMMENT ON TABLE/COLUMN metadata,
        # so it always looks "changed" to metadata_sync's skip_if_unchanged check on
        # the next run. Use "overwrite_in_place" instead if that matters and the
        # table's schema is stable.
        try:
            _drop_and_recreate_table(spark, table_name, query)
        except Exception as e:
    ...
    if table_type == "overwrite_in_place":
        print(
            f"\t📤 Overwriting {table_name} in place (preserves table/column comments if schema is unchanged)"
        )
        # A plain mode("overwrite") without overwriteSchema lets Delta enforce that the
        # incoming DataFrame's schema exactly matches the existing table's — it raises a
        # schema-mismatch error rather than silently unioning columns like
        # overwriteSchema=true would. On the happy path (no schema drift) the table
        # object itself is never dropped, so its COMMENT ON TABLE/COLUMN metadata
        # survives across runs and metadata_sync's skip_if_unchanged can actually skip
        # unchanged columns. If the schema really did drift, fall back to the same
        # drop+recreate behaviour as "replace" so the pipeline doesn't break.
        try:
            df = spark.sql(query).distinct()
            ...
            is_schema_drift = any(marker in err_msg for marker in schema_drift_markers)
            if not is_schema_drift:
                wrapped_msg = (
                    f">>> ⚠️ Error inserting into: {table_name} \n Exception: {e}"
                )
                print(wrapped_msg)
                raise RuntimeError(wrapped_msg) from e

            print(
                f"\t⚠️ Schema drift detected on {table_name}; falling back to drop+recreate "
                "(table/column comments will need to be reapplied)"
            )
            try:
                _drop_and_recreate_table(spark, table_name, query)
                ...
                print(wrapped_msg)
                raise RuntimeError(wrapped_msg) from e2
        return
    ...
    if not raw_match_keys:
        raise KeyError(
            f"\t> ⚠️ 'match_keys' missing in config for table '{table}'. Required for merge operations."
        )
    ...
    cnt, max_timestamp = check_if_table_is_empty(
        spark, target_tbl_prefix, table, models_config
    )

    if cnt > 0:
        where_clause = (
            f"WHERE {models_config[table]['load_date_column']} > '{max_timestamp}'"
        )
...
def process_table(
    ...
    try:
        if table_configuration[tgt_table]["code_type"] == "sql":
            ...
            if len(file_name) < 3:
                raise ValueError(
                    f"Invalid file_name '{file_name}' for table '{tgt_table}'."
                )
            ...
            tbl_type = table_configuration[tgt_table].get("table_type", "merge").lower()
            ...
            notebook_relative_path = os.path.relpath(
                os.path.join(
                    path_to_models, table_configuration[tgt_table]["file_name"]
                ),
                repo_root,
            ).replace(".ipynb", "")

            notebook_params = dict(
                table_configuration[tgt_table].get("notebook_params", {})
            )
            args = {
            ...
            dbutils.notebook.run(
                notebook_relative_path, timeout_seconds=1200, arguments=args
            )

        else:
            raise ValueError(
                f"Invalid code_type '{table_configuration[tgt_table]['code_type']}' for table '{tgt_table}'."
            )
...
def entry_table_loader(
    ...
    for level in range(1, max_level + 1):
        tables_to_run = [
            t for t in table_configuration if table_configuration[t]["level"] == level
        ]
        ...
        with ThreadPoolExecutor(max_workers=len(tables_to_run)) as executor:
            ...
            for future in as_completed(future_to_table):
                ...
                try:
                    ...
                    print(
                        f">>> ⚠️ Exception occurred while processing {table}:\n{traceback.format_exc()}"
                    )
                    if table not in failed_tables:
        ...
        print(
            f"\tProcessed: {successful_tables}"
            if successful_tables
            else "\t❌ No tables were successfully processed."
        )

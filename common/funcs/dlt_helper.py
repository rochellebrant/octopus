...
def generate_and_append_tombstones(
    spark,
    active_records_df: DataFrame,
    latest_snapshot_df: DataFrame,
    match_keys: list,
    tombstone_target_table: str,
    sequence_col: str = None,
):
    print(f"Executing Anti-Join on keys: {match_keys}")

    active_keys_df = active_records_df.select(*match_keys)
    snapshot_keys_df = latest_snapshot_df.select(*match_keys)

    tombstones_df = active_keys_df.join(
        snapshot_keys_df, on=match_keys, how="left_anti"
    )
    deleted_count = tombstones_df.count()

    # ALWAYS build the schema structure, even if empty
    # because streams needs to be setup to account for all potential streams
    # even if one might be empty
    final_tombstones = (
        tombstones_df.withColumn("is_deleted", f.lit(True))
        .withColumn("datalake_ingestion_timestamp", f.current_timestamp())
        .withColumn("table_id", f.lit(-1))
    )

    if sequence_col and sequence_col not in final_tombstones.columns:
        final_tombstones = final_tombstones.withColumn(
            sequence_col, f.current_timestamp()
        )

    if deleted_count > 0:
        # Append actual deletes
        (
            final_tombstones.write.format("delta")
            .mode("append")
            .option("mergeSchema", "true")
            .saveAsTable(tombstone_target_table)
        )

        print(
            f"✅ Successfully appended {deleted_count} tombstones to {tombstone_target_table}."
        )
    else:
        print(f"✅ No deleted records detected. 0 tombstones generated.")

        if not spark.catalog.tableExists(tombstone_target_table):
            print(
                f"🛠️ Initializing empty tombstone table to stabilize DLT stream topology: {tombstone_target_table}"
            )
            (
                final_tombstones.write.format("delta")
                .mode("append")
...
def cast_columns(
    ...
    if not column_definitions or table not in column_definitions:
        print(
            f"WARNING: Table '{table}' missing or no definitions provided. Skipping casting."
        )
        return df
    ...
    for defined_col in expected_columns.keys():
        if defined_col.lower() not in existing_cols_map:
            print(
                f"WARNING: Column '{defined_col}' defined for casting but not found in data for table '{table}'."
            )
    ...
    for user_col_name, col_data in expected_columns.items():
        if user_col_name.lower() in existing_cols_map:
            ...
            data_type = (
                col_data.get("schema", {}).get("data_type", "STRING").strip().upper()
            )
    ...
    timestamp_formats = {
        existing_cols_map.get(k.lower(), k): v
        for k, v in table_params.get("timestamp_formats", {}).items()
    }
    date_formats = {
        existing_cols_map.get(k.lower(), k): v
        for k, v in table_params.get("date_formats", {}).items()
    }
    ...
    for col, fmt in timestamp_formats.items():
        ...
        if expected_schema_cleaned.get(col, "") != "TIMESTAMP":
            raise ValueError(
                f"Column '{col}' in table '{table}' has a timestamp format but is defined as {expected_schema_cleaned.get(col, 'UNKNOWN')}."
            )
    ...
    for col, fmt in date_formats.items():
        ...
        if expected_schema_cleaned.get(col, "") != "DATE":
            raise ValueError(
                f"Column '{col}' in table '{table}' has a date format but is defined as {expected_schema_cleaned.get(col, 'UNKNOWN')}."
            )
    # ---------------------------------------
        ...
        if actual_name in expected_schema_cleaned:
            ...
            if target_type == "TIMESTAMP":
                if actual_name in timestamp_formats:
                    ...
                    select_exprs.append(
                        f.to_timestamp(clean_col, fmt).alias(user_case_name)
                    )
                else:
                    # Native cast naturally handles ISO 8601 strings (the 'T') without failing!
                    select_exprs.append(
                        clean_col.cast("timestamp").alias(user_case_name)
                    )
                ...
                if actual_name in date_formats:
                    ...
                    select_exprs.append(
                        f.to_date(f.to_timestamp(clean_col, fmt)).alias(user_case_name)
                    )
                else:
                    select_exprs.append(
                        f.to_date(clean_col.cast("timestamp")).alias(user_case_name)
                    )
...
def apply_column_comments(
    df: DataFrame, table: str, column_definitions: dict
) -> DataFrame:
    """
...
def build_live_schema(spark, input_table_full_path, table_key, column_definitions):
    ...
    try:
        ...
        print(
            f"WARNING: Could not read schema from '{input_table_full_path}'. "
        ...
        print(
            f"ERROR: Unexpected failure while reading schema from '{input_table_full_path}'."
        )
        raise e
    ...
    for defined_col_name, meta in table_def.items():
        if defined_col_name.lower() not in added_cols:
            ...
            if comment:
                schema_parts.append(
                    f"`{defined_col_name}` {col_type} COMMENT '{comment}'"
                )
            else:
...
def build_standard_schema(df: DataFrame, table: str, column_definitions: dict) -> str:
    ...
    for field in df.schema:
        ...
        if col_meta:
            comment = col_meta.get("description", {}).get("comment") or col_meta.get(
                "comment", ""
            )
            comment = str(comment).replace("'", "\\'") if comment else ""
        else:
...
def build_scd_schema(table: str, column_definitions: dict) -> str:
    ...
    if not table_def:
        print(
            f"WARNING: Table '{table}' missing from definitions. SCD table will lack comments."
        )
        return None
    ...
    schema_to_apply.append(
        '__START_AT TIMESTAMP COMMENT "When this record became live"'
    )
    schema_to_apply.append(
        '__END_AT TIMESTAMP COMMENT "When this record became no longer valid"'
    )
...
def apply_hard_check_flags(
    df, table_rules, table_prefix="", landing_db="", bronze_db="", silver_db=""
):
    """
    ...
    Each rule value is either a plain SQL string (legacy shorthand, always hard)
    or a dict {"expr": "<sql>", "hard_check": bool} to opt into a soft check.
    Hard failures are excluded from the clean table; soft failures are flagged
    for the quarantine union but still pass through to clean.
    """
    if not table_rules:
        return df

    def _parse_rule(rule_definition):
        if isinstance(rule_definition, dict):
            return rule_definition["expr"], rule_definition.get("hard_check", True)
        return rule_definition, True

    new_col_names = set()
    flag_cols = []
    for rule_name, rule_definition in table_rules.items():
        rule_logic, is_hard = _parse_rule(rule_definition)
        formatted_logic = rule_logic.format(
            PREFIX=table_prefix,
        ...
        col_name = f"{dq_prefix}{rule_name}"
        new_col_names.add(col_name)

        flag_cols.append(
            f.expr(f"CASE WHEN {formatted_logic} THEN 1 else 0 END").alias(col_name)
        )
...
def build_dup_count_df(df: DataFrame, key_cols: list[str]) -> DataFrame:
    ...
    """
    return df.groupBy(*key_cols).count().withColumnRenamed("count", "dup_count")


def flag_duplicates(
    stream_df: DataFrame, static_df: DataFrame, key_cols: list[str], flag_col: str
) -> DataFrame:
    """
    Appends a binary flag column marking rows in stream_df whose key_cols combination
    occurs more than once in static_df. Dup counts are computed via a static batch
    aggregation and broadcast-joined onto the streaming DataFrame, since Structured
    Streaming does not support joining a streaming aggregation back into a stream-stream join.

    Args:
        stream_df (DataFrame): The streaming row-level DataFrame to flag.
        static_df (DataFrame): A static batch read of the same source, used to compute dup counts.
        key_cols (list[str]): Columns defining the uniqueness key.
        flag_col (str): Name of the flag column to append.

    Returns:
        DataFrame: stream_df with an appended binary flag column.
    """
    dup_counts = (
        static_df.groupBy(*key_cols).count().withColumnRenamed("count", "__dup_count")
    )

    joined = stream_df.join(f.broadcast(dup_counts), on=key_cols, how="left")
    return joined.withColumn(
        flag_col, f.when(f.col("__dup_count") > 1, 1).otherwise(0)
    ).drop("__dup_count")
    ...
    for check_type, check_config in table_config[table][checks_field].items():
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to execute SQL for check '{check_type}': {e}"
                )

            join_keys_mapping = {
                k: v for k, v in check_config.items() if k != "hard_check"
            }
            ...
            joined_df = df.join(f.broadcast(ref_df), on=join_conditions, how="left")

            check_col = f"__ref_{right_keys[0]}"
            joined_df = joined_df.withColumn(
                flag_col_name, f.when(f.col(check_col).isNull(), 1).otherwise(0)
            )
...
def check_sql_reference_mismatches(spark, df, sql_queries, table_config, table):
    ...
    for validation_check_name, column_mapping in checks_config.items():
        ...
        if not sql_query:
            raise ValueError(
                f"❌ Check `{validation_check_name}` not found in imported SQL queries."
            )

        if not column_mapping or not isinstance(column_mapping, dict):
            raise ValueError(
                f"❌ No valid join conditions for `{validation_check_name}` in `{checks_field}`."
            )
        ...
        try:
            ...
            raise Exception(
                f"❌ Error executing SQL query for `{validation_check_name}`: {e}"
            )
        ...
        summary_df = (
            ...
            .withColumn("missing_values", f.concat_ws(", ", f.col("missing_values")))
        )
...
def filter_quarantine_clean(df: DataFrame) -> (DataFrame, DataFrame):
    ...
    clean_df = df.filter((f.col(f"{DQ_HARD_PREFIX}quarantine") == 0))

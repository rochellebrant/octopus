    ...
    for table_name in table_list:
        try:
            ...
            if delta_stats_exists:
                ...
                cutoff_index = (
                    df.filter(f.col("col_name") == "").select("row_num").first()[0]
                )
            ...
            formatted_columns = df.withColumn(
                ...
                f.when(
                    ...
                    f.concat_ws(
                        " ", f.col("col_name"), f.col("data_type"), f.lit("[PK]")
                    ),
                ).otherwise(f.concat_ws(" ", f.col("col_name"), f.col("data_type"))),
            )

            columns_list = [
                str(row["formatted_col"])
                for row in formatted_columns.select("formatted_col").collect()
            ]
    ...
    for table_name in table_list:
        try:
            ...
            df_fk = df_fk.withColumn(
                "source_column", f.regexp_extract(f.col("data_type"), r"\(`(.*?)`\)", 1)
            )
            df_fk = df_fk.withColumn(
                "target_column",
                f.regexp_extract(f.col("data_type"), r".*\((`?)(\w+)\1\)$", 2),
            )
            df_fk = df_fk.withColumn(
                "target_table",
                f.regexp_extract(
            ...
            reference_strings = [
                ...
                for row in df_fk.select("reference_string").collect()
            ]

...
def validate_is_dict(name: str, var: any) -> None:
    if not isinstance(var, dict):
        raise ValueError(
            f"⚠️ Missing or invalid `{name}`. Expected a dictionary. Received: {type(var)}."
        )
...
def validate_is_list(name: str, var: any) -> None:
    if not isinstance(var, list) or len(var) == 0:
        raise ValueError(
            f"⚠️ Missing or invalid `{name}`. Expected a non-empty list. Received: {type(var)}."
        )
...
def validate_is_string(name: str, var: any) -> None:
    if not isinstance(var, str):
        raise ValueError(
            f"⚠️ Missing or invalid `{name}`. Expected a string. Received: {type(var)}."
        )
...
def get_cdm_and_data_input_methods(check_config: dict, check_name: str):
    ...
    if (
        data_input_method not in valid_input_methods
        or cdm_input_method not in valid_input_methods
    ):
        raise ValueError(
            f'⚠️ Invalid `input_method.Data` or `input_method.CDM` for check {check_name}.\
...
def get_join_type(check_config: dict, check_name: str) -> str:
    ...
    if join_type not in valid_join_types:
        raise ValueError(
            f"⚠️ Invalid `join_type` for check `{check_name}`. "
...
def validate_join_columns_config(join_columns: list, check_name: str) -> None:
    ...
    if not primary_config:
        raise ValueError(
            f'⚠️ Expected "primary" key in `join_columns` for check {check_name}. Received: {primary_config.keys()}'
        )
    ...
    if "CDM" not in k or "Data" not in k:
        raise ValueError(
            f'⚠️ Expected "CDM" and "Data" keys in `join_columns.primary` for check {check_name}. Received: {k}'
        )
...
def validate_table_presence(
    table_name: str, tables_config: dict, check_name: str
) -> None:
    table = tables_config.get(table_name)
    if not table:
        raise ValueError(
            f"⚠️ Missing `tables.{table_name}` for check {check_name}. Expected a table in the format `catalog.database.table`. Received {table}."
        )


def validate_sql_queries_config(
    sql_queries_dict: dict, check_name: str, source_key: str
):
    validate_is_dict(f"{check_name}.input_method.sql_queries", sql_queries_dict)
    validate_is_string(
        f"{check_name}.input_method.sql_queries.{source_key}",
        sql_queries_dict[source_key],
    )


def get_full_name_space_target_table(
    check_name: str, prefix: str = None, database: str = None
):
    return f"{database}.{prefix}__cdm_checker__{check_name}"


def fetch_table_or_query(
    spark,
    fetch_method: str,
    check_config,
    check_name: str,
    ...
    if fetch_method == "table":
        ...
        raise ValueError(
            f"Invalid fetch method: {fetch_method}. Use 'table' or 'sql_query'."
        )
...
def ensure_required_columns(spark, check_name, required_cols, df, source_key: str):
    ...
    if missing_cols:
        raise ValueError(
            f"❌ Mismatched columns in check `{check_name}`: "
...
def get_column_join_mapping_dict(
    spark, primary_col_mapping_list: list, secondary_col_mapping_list: list
) -> tuple[dict, dict]:
...
def validate_summary_table_config(config):
    ...
    if "catalog" not in summary_table_config or "database" not in summary_table_config:
        raise ValueError(
            f'>>> Expected "catalog" and "database" keys in `summary_table.save_to`. Recieved: {summary_table_config.keys()}'
        )
...
def replace_limited_with_ltd(df, input_col, output_col):
    return df.withColumn(
        output_col, f.regexp_replace(f.col(input_col), "limited", "ltd")
    )
...
def add_secondary_messages_to_df(df, secondary_col_map, match_success_str="Match"):
    ...
    df = df.withColumn(
        "secondary_join_message",
        f.when(
            match_found_condition,
...
def process_secondary_conditions(
    spark, df, secondary_column_mapping_dict, cdm_prefix, match_success_str="Match"
):
    # Generate mismatch conditions
    ...
    # Add messages to DataFrame
    df = add_secondary_messages_to_df(
        df, secondary_column_mapping_dict, match_success_str
    )
...
def add_column_mapping_to_df(spark, df, column_mapping, column_type="primary"):
    ...
    column_mapping = ",\n".join(
        [f"{left_col} ↔ {right_col}" for left_col, right_col in column_mapping.items()]
    )
...
def reorder_columns(spark, df, column_order, optional_columns):
    ...
    existing_optional_columns = [
        col_name for col_name in optional_columns if col_name in df.columns
    ]
    remaining_columns = [
        col_name
        for col_name in df.columns
        if col_name not in column_order + existing_optional_columns
    ]
...
def count_missing_values(spark, df, column_mapping, right_prefix):
    ...
    """
    missing_in_right_expr = " OR ".join(
        [
            f"{right_prefix}{right_col} IS NULL AND {left_col} IS NOT NULL"
            for left_col, right_col in column_mapping.items()
        ]
    )
    missing_in_left_expr = " OR ".join(
        [
            f"{left_col} IS NULL AND {right_prefix}{right_col} IS NOT NULL"
            for left_col, right_col in column_mapping.items()
        ]
    )
    both_missing_expr = " OR ".join(
        [
            f"{left_col} IS NULL AND {right_prefix}{right_col} IS NULL"
            for left_col, right_col in column_mapping.items()
        ]
    )

    # Count missing values
    missing_in_right_count = (
        df.filter(f.expr(missing_in_right_expr)).count() if missing_in_right_expr else 0
    )
    missing_in_left_count = (
        df.filter(f.expr(missing_in_left_expr)).count() if missing_in_left_expr else 0
    )
    both_missing_count = (
        df.filter(f.expr(both_missing_expr)).count() if both_missing_expr else 0
    )
...
def convert_timestamps(spark, df, timestamp_formats):
    ...
    for col, fmt in timestamp_formats.items():
        if col in df.columns:
            df = df.withColumn(
                col, f.date_format(f.to_timestamp(df[col], fmt), "yyyy-MM-dd HH:mm:ss")
            )
    return df
...
def convert_dates(df, date_formats):
    ...
    for col, format_str in date_formats.items():
        if col in df.columns:
            df = df.withColumn(col, f.to_date(df[col], format_str, "yyyy-MM-dd"))
    return df

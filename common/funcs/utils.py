...
def load_config_module(
    module_name: str, config_attrs: Union[str, List[str]], default_val: Any = None
) -> Any:
    """
    Dynamically imports a module and searches for one or more possible attribute names.

    Args:
        module_name (str): Name of the module to import.
        config_attrs (str | List[str]): A single attribute name or a list of names to try.
        default_val (Any): Value to return if nothing is found.
    """
    if default_val is None:
        default_val = {}

    if not module_name or not module_name.strip():
        print(f">>> Skipping: No module name provided.")
        return default_val

    # Ensure config_attrs is a list for consistent iteration
    attrs_to_try = [config_attrs] if isinstance(config_attrs, str) else config_attrs

    try:
        print(f">>> Attempting to load module: {module_name}")
        mod = importlib.import_module(module_name)

        # Iteratively look for the first matching attribute
        for attr in attrs_to_try:
            if hasattr(mod, attr):
                print(f">>> Success: Found attribute '{attr}' in '{module_name}'.")
                return getattr(mod, attr)

        print(f">>> WARNING: None of {attrs_to_try} found in '{module_name}'.")
        return default_val

    except ImportError:
        print(f">>> WARNING: Module '{module_name}' not found. Using default.")
        return default_val
    except Exception as e:
        print(f">>> ERROR: Unexpected error loading '{module_name}': {e}")
        raise e
...
def _quote_3part(name: str) -> str:
    ...
    if len(parts) != 3:
        raise Exception(
            f">>> ⚠️ target must be 3-part: catalog.schema.table, got: {name}"
        )
...
def get_table_schema(spark, fq_schema: str, table_name: str) -> DataFrame:
    ...
    try:
        ...
        raise Exception(
            f">>> Error getting table schema for {fq_schema}.{table_name}: {e}"
        )


def generate_table_list(
    spark, catalog_name: str, schema_name: str, table_type: str = "MANAGED"
) -> List[str]:
    """
    ...
    table_list = table_list.filter(f.col("table_type") == table_type).select(
        "table_name"
    )
...
def generate_table_key_data(
    spark, tables: List[str], SCHEMA: str
) -> Optional[DataFrame]:
    """
...
def generate_fk_script_df(accumulated_df: DataFrame, SCHEMA: str) -> DataFrame:
    ...
    accumulated_df_fk = accumulated_df_fk.withColumn(
        "fk_col",
        f.regexp_extract(f.col("key"), r"FOREIGN KEY\s+\(`?([^`\s\)]+)`?\)", 1),
    )

    # Clean up the constraint string
    # Take the raw 'key' string from DESCRIBE TABLE (e.g. "FOREIGN KEY (col) REFERENCES table(id)")
    accumulated_df_fk = accumulated_df_fk.withColumn("constraint", f.col("key"))
...
def ensure_historic_tables_exist(
    ...
    """
    For every logical source table referenced in table_config that has an entry in
    historic_notebook_map, checks whether its physical (prefixed) table exists.
    If missing, runs the mapped notebook (dbutils.notebook.run) to create it.
    """
    required_sources = {src for sources in table_config.values() for src in sources}

    for source in required_sources:
        if source not in historic_notebook_map:
            continue

        physical_name = f"{database}.{prefix}{source}"
        if table_exists(spark, physical_name):
            print(
                f">>> Historic table already exists, skipping creation: {physical_name}"
            )
            continue

        notebook_name = historic_notebook_map[source]
        print(
            f">>> Historic table missing: {physical_name}. Running notebook: {notebook_name}"
        )
        try:
            dbutils.notebook.run(notebook_name, timeout_seconds, notebook_params or {})
            print(f">>> Created historic table via notebook: {notebook_name}")
        except Exception as e:
            raise Exception(
                f">>> ⚠️ Failed to create missing historic table '{physical_name}' "
                f"via notebook '{notebook_name}': {e}"
            ) from e


def apply_env_prefixes(mapping: dict, database: str, prefix: str) -> dict:
    """
    Translates logical table mappings into physical environment-specific paths.
    """
    resolved_mapping = {}
    for target, sources in mapping.items():
        # Format the target table
        resolved_target = f"{database}.{prefix}{target}"

        # Format all source tables
        resolved_sources = [f"{database}.{prefix}{src}" for src in sources]

        resolved_mapping[resolved_target] = resolved_sources
    ...
    try:
        ...
        if delete_mode == "scoped":
            if not period_column:
                raise Exception(
                    ">>> ⚠️ period_column is required when delete_mode='scoped'."
                )
            if period_column not in df.columns:
                raise Exception(
                    f">>> ⚠️ period_column '{period_column}' not found in DataFrame."
                )

        # Optionally include period in match keys
        if (
            delete_mode == "scoped"
            and include_period_in_keys
            and period_column not in match_keys
        ):
        ...
        if not table_exists(spark, target):
            ...
            spark.sql(
                f"CREATE TABLE IF NOT EXISTS {target} USING DELTA AS SELECT * FROM `{view_name}` WHERE 1=0"
            )
        ...
        if new_cols:
            ...
            add_cols = ", ".join(
                f"`{c}` {df.schema[c].dataType.simpleString()}" for c in new_cols
            )
        ...
        if update_cols:
            upd_cond = " OR ".join(
                [f"NOT (tgt.`{c}` <=> src.`{c}`)" for c in update_cols]
            )
        ...
        if delete_mode in ("scoped", "full"):
            if delete_mode == "scoped":
                if scope_values is None:
                    scope_values = [
                        r[0] for r in df.select(period_column).distinct().collect()
                    ]
                if not scope_values:
                    raise Exception(
                        f">>> ⚠️ No values for period_column '{period_column}' found in batch."
                    )
            ...
            predelete_match_cond = " AND ".join(
                [f"(tgt.`{k}` <=> src.`{k}`)" for k in match_keys]
            )

...
def test_apply_comments_missing_table_in_definitions(
    mock_warn, mock_spark, mock_result_row
):
...
@patch("libraries.metadata_sync.applier.diff_columns", return_value=(set(), set()))
@patch(
    "libraries.metadata_sync.applier.get_table_columns", return_value={"2000", "col1"}
)
def test_apply_comments_quotes_numeric_column_names(
    mock_get_cols, mock_diff, mock_spark, mock_result_row
):
    """Regression test: all-digit column names (e.g. pivoted year columns) must be
    backtick-quoted or Spark's SQL parser raises PARSE_SYNTAX_ERROR on the bare numeral.
    """
    col_defs = {
        "tbl": {
            "table": {"comment": "Main table comment"},
            "columns": {
                ...
                "2000": {
                    "description": {
                        "comment": "Year 2000 value",
                        "unit": "GBP",
                        "long_name": "2000 Value",
                    }
                },
            },
        }
    ...
    assert any("COMMENT ON COLUMN cat.db.tbl.`2000` IS" in sql for sql in all_sql)
    assert not any(".2000 IS" in sql for sql in all_sql)


@patch("libraries.metadata_sync.applier.diff_columns", return_value=(set(), set()))
@patch("libraries.metadata_sync.applier.get_table_columns", return_value={"col1"})
@patch("libraries.metadata_sync.applier.get_existing_column_comments")
@patch("libraries.metadata_sync.applier.get_existing_table_comment")
def test_apply_comments_skip_if_unchanged(
    mock_get_existing_table,
    mock_get_existing_cols,
    mock_get_cols,
    mock_diff,
    mock_spark,
    mock_result_row,
):
    """When skip_if_unchanged=True and comments already match, no COMMENT ON statements should run."""
    col_defs = {
        "tbl": {
            "table": {"comment": "Main table comment"},
            "columns": {
    ...
    mock_get_existing_table.return_value = "Main table comment"
    mock_get_existing_cols.return_value = {"col1": "Weight. Col comment \nUNIT: kg"}
...
@patch(
    "libraries.metadata_sync.applier.get_table_columns", return_value={"col1", "col2"}
)
...
@patch(
    "libraries.metadata_sync.applier.preflight", return_value=set()
)  # Preflight finds NO tables
...
def test_apply_all_missing_table_fail(mock_preflight, mock_spark):
    ...
    with pytest.raises(
        TableCommentsError, match="Comment application failed with 1 error"
    ):
        apply_comments_for_all_tables(
            mock_spark,
            "cat",
...
@patch(
    "libraries.metadata_sync.applier.apply_comments_from_definitions",
    side_effect=Exception("API Outage"),
)
@patch("libraries.metadata_sync.applier.preflight", return_value={"tbl1", "tbl2"})
def test_apply_all_exception_handling_fail_fast(
    mock_preflight, mock_apply_def, mock_spark
):
    ...
    with pytest.raises(TableCommentsError):
        apply_comments_for_all_tables(
            mock_spark, "cat", "db", col_defs, fail_fast=True, fail_on_any_error=True
        )

...
from libraries.metadata_sync.validation import (
    ...
    get_table_columns,
    diff_columns,
    get_existing_table_comment,
    get_existing_column_comments,
)
...
def test_quote_identifier():
    # All-digit names (e.g. pivoted year columns) must be backtick-quoted
    assert quote_identifier("2000") == "`2000`"
    # Ordinary identifiers are still wrapped, which is harmless in Spark SQL
    assert quote_identifier("col1") == "`col1`"
    # Embedded backticks are doubled per SQL identifier-escaping rules
    assert quote_identifier("weird`col") == "`weird``col`"
...
def test_validate_shape_happy_path():
    ...
    valid_config = {
        "table_name": {"columns": {"id": {"description": {"comment": "Primary Key"}}}}
    }
...
def test_validate_shape_invalid_nested_types():
    ...
    with pytest.raises(TypeError, match="must be a dict"):
        validate_column_definitions_shape(
            {"tbl": {"columns": {"id": {"description": "not a dict"}}}}
        )
...
def test_preflight_spark_error(mock_list_tables, mock_spark):
    ...
    with pytest.raises(
        TableCommentsError, match="Preflight failed: cannot access schema"
    ):
...
def test_preflight_require_tables_failure(mock_list_tables, mock_spark):
    ...
    with pytest.raises(
        TableCommentsError,
        match="Preflight failed: none of the tables in column_definitions exist",
    ):
...
# ==========================================
# TESTS: Existing Comment Lookups (skip_if_unchanged support)
# ==========================================


def test_get_existing_table_comment_found(mock_spark):
    mock_spark.sql.return_value.collect.return_value = [{"comment": "Existing comment"}]

    result = get_existing_table_comment(mock_spark, "cat", "db", "tbl")

    assert result == "Existing comment"
    sql_arg = mock_spark.sql.call_args[0][0]
    assert "cat.information_schema.tables" in sql_arg
    assert "table_schema = 'db'" in sql_arg
    assert "table_name = 'tbl'" in sql_arg


def test_get_existing_table_comment_missing(mock_spark):
    mock_spark.sql.return_value.collect.return_value = []

    assert get_existing_table_comment(mock_spark, "cat", "db", "tbl") is None


def test_get_existing_column_comments(mock_spark):
    row1, row2 = MagicMock(), MagicMock()
    row1.__getitem__.side_effect = lambda k: {
        "column_name": "id",
        "comment": "Primary key",
    }[k]
    row2.__getitem__.side_effect = lambda k: {"column_name": "name", "comment": None}[k]
    mock_spark.sql.return_value.collect.return_value = [row1, row2]

    result = get_existing_column_comments(mock_spark, "cat", "db", "tbl")

    assert result == {"id": "Primary key", "name": None}
    sql_arg = mock_spark.sql.call_args[0][0]
    assert "cat.information_schema.columns" in sql_arg

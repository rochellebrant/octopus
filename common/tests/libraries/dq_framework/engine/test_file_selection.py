...
from libraries.dq_framework.engine.file_selection import (
    parse_patterns,
    discover_file_names,
)
...
def test_discover_file_names_dir_not_found(mock_exists):
    ...
    with pytest.raises(
        NotADirectoryError, match="The configuration directory does not exist"
    ):
...
def test_discover_file_names_wildcards_and_exclude(mock_glob, mock_exists):
    ...
    f1.stem = "finance_orders"
    f2 = MagicMock(spec=Path)
    f2.stem = "finance_users"
    f3 = MagicMock(spec=Path)
    f3.stem = "hr_employees"

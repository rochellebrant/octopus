...
def test_load_combined_config_success(mock_load):
    ...
    defaults = {"roles": {"admin": "user1"}, "checks": [{"name": "default_check"}]}
    table_cfg = {
    ...
    result = load_combined_config(
        base_path="/base", config_group="finance", table_yaml_name="orders"
    )

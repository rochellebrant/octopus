...
def get_fqtn(table_cfg):
    ...
    if not fqtn:
        ...
        raise ValueError(
            f"FQTN missing for {table_name}. Ensure 'source_db' is set in the runner."
        )
    return fqtn
...
def _get_scd2_context(params, table_cfg):
    ...
    if not bk_val:
        raise ValueError(
            f"SCD2 checks require 'system_keys' role for {table_cfg.get('table_name')}"
        )
...
def is_not_null(params, table_cfg):
    ...
    roles_to_pull = params.get("columns_from_roles") or [
        params.get("columns_from_role")
    ]
    ...
    if not columns_to_check:
        raise ValueError(
            f"is_not_null check on {table_cfg.get('table_name')} has no columns to check."
        )
...
def pk_unique(params, table_cfg):
    cols = params.get("columns") or resolve_role(
        table_cfg, params.get("columns_from_role")
    )
    if not cols:
        raise ValueError(
            f"pk_unique check on {table_cfg.get('table_name')} missing target columns."
        )
    return [
        {
...
def is_in_range(params, table_cfg):
    ...
    if not columns_to_check:
        raise ValueError(
            f"is_in_range check on {table_cfg.get('table_name')} missing target columns."
        )
...
def no_future_timestamps(params, table_cfg):
    ...
    if not columns_to_check:
        raise ValueError(
            f"no_future_timestamps on {table_cfg.get('table_name')} missing target columns."
        )
...
def string_pattern_match(params, table_cfg):
    ...
    if not columns_to_check or not pattern:
        raise ValueError(
            f"string_pattern_match on {table_cfg.get('table_name')} requires 'columns' and 'pattern'."
        )
...
def fk_exists(params, table_cfg):
    ...
    ref_cols = params.get(
        "ref_columns"
    )  # The target aliases (what we want to call them)
    fqtn = get_fqtn(table_cfg)
    ...
    if not all([fk_cols, ref_tables, ref_cols, fqtn]):
        raise ValueError(
            f"fk_exists on {table_cfg.get('table_name')} missing required params."
        )
    ...
    for i, table_path in enumerate(resolved_refs):
        select_clause = ", ".join(
            [f"{src} AS {target}" for src, target in zip(source_ref_cols[i], ref_cols)]
        )
...
def scd2_pk_unique(params, table_cfg):
    ...
    key_role = (
        "business_keys" if "business" in params.get("name", "") else "system_keys"
    )
    ...
    if not keys or not start_col:
        raise ValueError(
            f"scd2_pk_unique on {table_cfg.get('table_name')} missing keys or start_col."
        )

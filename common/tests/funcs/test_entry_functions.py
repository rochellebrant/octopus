...
def test_load_entry_tables_missing_columns():
    ...
    with patch.object(
        target=spark.catalog, attribute="listColumns", side_effect=fake_list_columns
    ):
        try:
            load_entry_tables(
                spark,
...
def test_load_entry_tables_non_empty():
    ...
    with patch.object(
        spark.catalog, "listColumns", side_effect=fake_list_columns
    ), patch(
        "funcs.entry_table_functions.check_if_table_is_empty",
        return_value=(42, "2024-01-01 12:00:00"),
    ):
        ...
        def fake_sql(query):
            ...
            return spark.createDataFrame(
                [("dummy",)], StructType([StructField("dummy", StringType(), True)])
            )
...
def test_load_entry_tables_empty():
    ...
    with patch.object(
        ...
        "funcs.entry_table_functions.check_if_table_is_empty", return_value=(0, None)
    ):
...
def test_load_entry_tables_merge_exception():
    ...
    with patch.object(
        ...
        with patch.object(
            spark, "sql", side_effect=Exception("Simulated Spark failure")
        ) as sql_mock:
            try:
                load_entry_tables(
                    spark,
...
# ---------------------------------------------------------------------------
# Test A6: load_entry_tables - "overwrite_in_place" happy path (no drift)
# ---------------------------------------------------------------------------
@suppress_print
def test_load_entry_tables_overwrite_in_place_happy_path():
    models_config = {"test_table": {"table_type": "overwrite_in_place"}}
    ...
    def fake_sql_returning_df(query):
        queries_executed.append(query)
        return FakeDF()

    with patch.object(spark, "sql", side_effect=fake_sql_returning_df):
        load_entry_tables(
            spark,
            models_config,
            "test_table",
    ...
    assert not any(
        "DROP TABLE" in q for q in queries_executed
    ), "Should not drop the table on the happy path"
    assert any("GRANT MANAGE ON tgt_test_table" in q for q in queries_executed)
    write_mock.format.assert_called_once_with("delta")
    write_mock.format.return_value.mode.assert_called_once_with("overwrite")
    write_mock.format.return_value.mode.return_value.option.assert_not_called()
    write_mock.format.return_value.mode.return_value.saveAsTable.assert_called_once_with(
        "tgt_test_table"
    )


# ---------------------------------------------------------------------------
# Test A7: load_entry_tables - "overwrite_in_place" falls back on schema drift
# ---------------------------------------------------------------------------
@suppress_print
def test_load_entry_tables_overwrite_in_place_schema_drift_fallback():
    models_config = {"test_table": {"table_type": "overwrite_in_place"}}
    ...
    class FakeDF:
        ...
        class write:
            ...
            def format(fmt):
                ...
                def raise_schema_mismatch(_table):
                    raise Exception(
                        "A schema mismatch detected when writing to the Delta table."
                    )

                m.mode.return_value.saveAsTable.side_effect = raise_schema_mismatch
                return m
    ...
    assert any(
        "DROP TABLE IF EXISTS tgt_test_table" in q for q in queries_executed
    ), "Schema drift should trigger fallback to drop+recreate"


# ---------------------------------------------------------------------------
# Test A8: load_entry_tables - "overwrite_in_place" re-raises non-drift errors
# ---------------------------------------------------------------------------
@suppress_print
def test_load_entry_tables_overwrite_in_place_non_drift_error_raises():
    models_config = {"test_table": {"table_type": "overwrite_in_place"}}
    ...
    class FakeDF:
        ...
        class write:
            ...
            def format(fmt):
                ...
                def raise_unrelated_error(_table):
                    raise Exception("Some unrelated permission error")

                m.mode.return_value.saveAsTable.side_effect = raise_unrelated_error
                return m
    ...
    with patch.object(spark, "sql", side_effect=fake_sql):
        try:
            ...
            assert (
                False
            ), "Expected a RuntimeError to propagate for a non-schema-drift error."
        except RuntimeError as e:
            assert "Error inserting into" in str(e)

    assert not any(
        "DROP TABLE" in q for q in queries_executed
    ), "A non-schema-drift error should not trigger the drop+recreate fallback"
...
def test_process_table_sql_replace():
    ...
    ) as mock_init, patch("funcs.entry_table_functions.load_entry_tables") as mock_load:
    ...
    mock_init.assert_called_once_with(
        ...
        create_mode="CREATE TABLE IF NOT EXISTS",  # <--- Updated to match our stream-safe fix!
...
def test_process_table_notebook():
    ...
    with patch("os.getcwd", return_value=fake_cwd), patch(
        "os.path.relpath", return_value=fake_relpath
...
def test_process_table_unrecognised_code_type():
    ...
    with patch(
        ...
        "funcs.entry_table_functions.dbutils"
    ) as mock_dbutils:
    ...
    assert (
        "test_table" in failed_tables
    ), "Unknown code_type should be logged in failed_tables."
...
def test_process_table_missing_model_script():
    ...
    assert (
        "test_table" in failed_tables
    ), "Missing script should cause failure and be logged in failed_tables."
...
def test_process_table_notebook_invalid_path_run_failure():
    ...
    with patch("os.getcwd", return_value="/fake/repo"), patch(
        "os.path.relpath", return_value="notebooks/broken_notebook.ipynb"
    ), patch("funcs.entry_table_functions.dbutils") as mock_dbutils:
    ...
    assert (
        "test_table" in failed_tables
    ), "Notebook run failure should log table in failed_tables."
...
def test_process_table_sql_helper_failure():
    ...
    assert (
        "test_table" in failed_tables
    ), "Failure inside helper should be surfaced via failed_tables."
...
def test_process_table_notebook_with_params():
    ...
    with patch("os.getcwd", return_value="/repo"), patch(
        "os.path.relpath", return_value="nb/params_notebook.ipynb"
    ), patch("funcs.entry_table_functions.dbutils") as mock_dbutils:
...
def test_process_table_notebook_none_prefixes():
    ...
    with patch("os.getcwd", return_value="/repo"), patch(
        "os.path.relpath", return_value="nb/prefixes.ipynb"
    ), patch("funcs.entry_table_functions.dbutils") as mock_dbutils:
...
def test_entry_table_loader():
    ...
    try:
        with patch(
            "funcs.entry_table_functions.process_table", side_effect=fake_process_table
        ):
            try:
                entry_table_loader(
                    spark,
                ...
                raise AssertionError(
                    "Expected TableProcessingError at end-of-run, but no exception was raised."
                )
...
def test_entry_table_loader_end_of_run_failure():
    ...
    assert "Level 2 Processing Summary:" in output, "Missing Level 2 summary"
    assert "Processed: ['table3']" in output, "Expected table3 as successful in level 2"
    assert ("\t No failures." in output) or (
        "Failures: []" in output
    ), "Expected no failures in level 2"
...
def test_import_sql_scripts_skips_dunder_init():
    models_dict = {}
    with patch("os.listdir", return_value=["__init__.py"]), patch(
        "importlib.import_module"
    ) as mock_import:
...
def test_import_sql_scripts_skips_parameter_file():
    models_dict = {}
    with patch("os.listdir", return_value=["some_parameters.py"]), patch(
        "importlib.import_module"
    ) as mock_import:
...
def test_import_sql_scripts_imports_multiple_attributes():
    models_dict = {}
    with patch("os.listdir", return_value=["multi_attr.py"]), patch(
        "importlib.import_module"
    ) as mock_import:
...
def test_import_sql_scripts_failure():
    models_dict = {}
    with patch("os.listdir", return_value=["bad_module.py"]), patch(
...
def test_import_sql_scripts_imports_no_attributes():
    models_dict = {}
    with patch("os.listdir", return_value=["empty_module.py"]), patch(
        "importlib.import_module"
    ) as mock_import:
...
def test_abort_if_failures_raises_with_count_and_context():
    ...
    try:
        ...
        raise AssertionError(
            "Expected TableProcessingError, but no exception was raised."
        )
    except TableProcessingError as e:
        msg = str(e)

...
def _patched_fake_f():
    ...
    if not hasattr(fake_f, "row_number"):
        ...
        rn._expr = "<row_number>"
        rn.over.side_effect = lambda w: MagicMock(
            name="row_number_over", _expr="<row_number_over>"
        )
...
def test_init_empty_tables_multi_success():
    ...
    create_statements = [
        c[0][0] for c in spark.sql.call_args_list if "CREATE TABLE" in c[0][0]
    ]
...
def test_init_empty_tables_drop_failure():
    ...
    assert (
        spark.sql.call_count == 1
    ), "CREATE/GRANT should be skipped after drop failure"
...
def test_mixed_data_loader_single_kpi_success():
    ...
    with patch("funcs.general_functions.f", _patched_fake_f()), patch(
        "funcs.general_functions.w", fake_w
    ):
        mixed_data_loader(
            spark,
            table_config=cfg,

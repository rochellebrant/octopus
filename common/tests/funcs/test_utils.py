...
# Set up repo path so 'funcs' can be imported
# cwd = os.getcwd()
...
def test_ensure_dict_key_prefix():
    ...
    assert ensure_dict_key_prefix({"dev_gold_model_table": 1}, prefix) == {
        "dev_gold_model_table": 1
    }

    # 3. Smart overlap detection (partial prefix already there)
    assert ensure_dict_key_prefix({"gold_model_aum_bridge": 1}, prefix) == {
        "dev_gold_model_aum_bridge": 1
    }
    assert ensure_dict_key_prefix({"model_aum_bridge": 1}, prefix) == {
        "dev_gold_model_aum_bridge": 1
    }

    # 4. No overlap (prefix prepended fully)
    assert ensure_dict_key_prefix({"aum_bridge": 1}, prefix) == {
        "dev_gold_model_aum_bridge": 1
    }
...
def test_get_table_schema():
    ...
    assert "Error getting table schema for cat.schema.my_table: Not found" in str(
        exc.value
    )
...
def test_overwrite_to_delta_writes_with_overwriteSchema():
    ...
    df.write.format.return_value.mode.return_value.option.assert_called_with(
...
def test_unify_tables_unions_and_overwrites_target():
    ...
    df1.write.format.return_value.mode.return_value.saveAsTable.assert_called_with(
        "tgt.all"
    )

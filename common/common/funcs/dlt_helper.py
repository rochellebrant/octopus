...
def apply_hard_check_flags(df, table_rules):
    """
    Evaluates the DataFrame against custom SQL expressions and appends 
    binary flag columns (`DQ_hard_...`) indicating if the rules are breached.
    ...
    Returns:
        DataFrame: Transformed DataFrame with appended hard check flags.
    """
    if not table_rules:
        return df
    ...
    flag_cols = [
        f.expr(f"CASE WHEN {rule_logic} THEN 1 else 0 END").alias(f"{DQ_HARD_PREFIX}{rule_name}")
...
def filter_quarantine_clean(df: DataFrame) -> (DataFrame, DataFrame):
    ...
    clean_df = df.filter(
        ...
        (f.col(f"{DQ_SOFT_PREFIX}quarantine") == 0)
    )

...
# 1. SETUP REPO PATH (Ensures we look in your dev area)
cwd = os.getcwd()
repo_root = os.path.abspath(os.path.join(cwd, ".."))
if repo_root not in sys.path:
    sys.path.insert(0, repo_root)

# 2. FORCE RELOAD (The "Proper" way to handle dev imports)
import funcs.dlt_helper as dlt_helper
importlib.reload(dlt_helper)
...
if __name__ == "__main__":
    ...
    test_cases = sorted([name for name in all_names if name.startswith("test_") and callable(globals()[name])])

    print(f"🚀 Starting DLT Helper Test Suite ({len(test_cases)} tests)...\n")
    print("-" * 60)

...
# %pip install --upgrade databricks-labs-dqx 'databricks-labs-dqx[llm]'
...
if BUNDLE_TARGET == "dev":
    ...
    if not REPO_ROOT.startswith("/Workspace"):
        REPO_ROOT = f"/Workspace{REPO_ROOT}"
            ...
            if any(fnmatch(table_name, pattern) for pattern in include_pats):
                if not (exclude_pats and any(fnmatch(table_name, pattern) for pattern in exclude_pats)):
                    ...
                    nbs = [
                        ...
                        if f.endswith(('.py', '.ipynb')) or '.' not in f
                    ]
    ...
    for table in all_tables:
        ...
        if table in bespoke_tables_dict and RUN_TYPE in ["all", "bespoke"]:
            for nb_name in bespoke_tables_dict[table]:
                ...
                # Use os.path.join to securely build the path.
                # Because bespoke_rel_dir already starts with a "/", this perfectly 
                # builds "/Repos/..." without injecting an accidental double slash.
                nb_path = os.path.join(bespoke_rel_dir, table, nb_name)

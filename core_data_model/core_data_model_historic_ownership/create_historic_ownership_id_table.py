...
# Explicit cycle flag: a genuine cycle means some company appears more than once in the
# deduped chain (consecutive-dup collapsing above already handles A,A,B; a repeat surviving
# that means the chain returns to a company it already visited, e.g. A->B->A).
df = df.withColumn(
    "has_cycle",
    F.size(F.col("company_chain_id")) != F.size(F.array_distinct(F.col("company_chain_id")))
)

df = df.withColumn(
    "parent_child_pairs",
    F.when(
        (F.size("company_chain_id") >= 2) & (~F.col("has_cycle")),
        F.expr("""
          transform(
            arrays_zip(
...
# Explicit self-loop filter: drop any pair where parent == child. This shouldn't occur after
# consecutive-dup collapsing above, but we guard for it explicitly rather than relying on that
# as a side effect.
df = df.withColumn(
    "parent_child_pairs",
    F.filter(F.col("parent_child_pairs"), lambda p: p[0] != p[1])
)
...
# Multiply edge percentages (0-1 fraction scale) to a single path percentage per (full_rel_id, segment)
...
grouped_segments_with_path_pct = (
    ...
    # Compute product of fractions across all edges in the path
    .withColumn(
        "path_fraction",
        F.when(
...
def apply_priority_and_intervals(raw_df, priority_df, id_col_prefix):
    ...
    The Phase Grouping: Next, group the data to create the intervals. We group by asset_id, lifecycle_phase_id, a boolean is_future flag, a conditional _grp_date, and a historical run id.
    ...
    # 2. Collapse to Phase Intervals
    df = df.withColumn("is_future", F.col("event_date") > TODAY)

    # 2a. Detect contiguous runs of the same phase among HISTORICAL milestones, ordered by
    # event_date. Without this, a phase that recurs non-contiguously (e.g. A -> B -> A) would
    # collapse its second occurrence into the FIRST A interval, since grouping on phase_id alone
    # has no notion of occurrence order for historical rows (whose _grp_date is always NULL).
    w_hist_run = W.partitionBy("asset_id").orderBy("event_date")
    _prev_phase = F.lag(phase_id_col).over(w_hist_run)
    _phase_changed = _prev_phase.isNull() | (_prev_phase != F.col(phase_id_col))
    df = df.withColumn(
        "_hist_run_id",
        F.when(~F.col("is_future"), F.sum(F.when(_phase_changed, 1).otherwise(0)).over(w_hist_run))
    )

    df = df.withColumn("_grp_date", F.when(F.col("is_future"), F.col("event_date")).otherwise(F.lit(None)))

    phase_intervals = (
        df.groupBy("asset_id", phase_id_col, "is_future", "_grp_date", "_hist_run_id")
        .agg(
    ...
    return timeline.drop("phase_start_date", "is_future", "_grp_date", "_hist_run_id")
...
# comment the .cache() out if you're running on serverless
...
rel_bounds = TemporalToolkit.build_boundaries_from_join(
    anchor_base,
    rels_for_bounds,
    join_cond=(
...
# -------------------------------
ai_bounds = TemporalToolkit.build_boundaries_from_join(
    anchor_base,
...
# 3) Portfolio version
# -------------------------------
pf_bounds = TemporalToolkit.build_boundaries_from_join(
    anchor_base,
    invp_ver_map,
    join_cond=(
...
# -------------------------------
lfe_bounds = TemporalToolkit.build_boundaries_from_join(
    anchor_base,
    lfe,
...
own_bounds = TemporalToolkit.build_boundaries_from_join(
    anchor_base,
    own,
...
def dedupe_segment_rows(df, key_cols, unit_cols=None, flag_specs=None, pass_through_cols=None):
    """
    Deterministically collapse multiple candidate rows sharing the same key_cols down to exactly
    one row per key.

    Delegates to TemporalToolkit.dedupe_segment_rows, which does this via a single hash
    aggregation (min_by/max_by) instead of the window+groupBy+join pattern this notebook used
    to hand-roll here directly -- same determinism guarantees (fewest-NULLs-among-unit_cols,
    hash tie-break, OR'd flags across all candidates), fewer shuffles per stage.
    """
    return TemporalToolkit.dedupe_segment_rows(
        df, key_cols, unit_cols=unit_cols, flag_specs=flag_specs, pass_through_cols=pass_through_cols
    )
...
seg_map = dedupe_segment_rows(
    seg_map_raw, GB_COLS,
    pass_through_cols=["ownership","pairs_with_ownership","has_relpath"],
    flag_specs={"has_asst_invp_map": "hit_ai"}
)
...
seg_invp = dedupe_segment_rows(
    seg_invp_raw, GB_COLS,
    pass_through_cols=["ownership","pairs_with_ownership","has_relpath","has_asst_invp_map"],
    flag_specs={"has_invp_dim": "hit_dim", "has_invp_bridge": "hit_brg", "has_invp_version": "hit_ver"}
)

# Lifecycle milestone covering the whole segment.
# Joined and deduped in its OWN stage (rather than combined with ownership below in one groupBy)
# so a milestone's id/type/phase are always picked together as a unit -- never mixed field-by-field
# with a different lifecycle candidate, or with an unrelated ownership candidate from the next stage.
seg_lfe_raw = (
    seg_invp.alias("s")
    .join(
        lfe.alias("l"),
...
seg_lfe = dedupe_segment_rows(
    seg_lfe_raw, GB_COLS,
    unit_cols=["lifecycle_milestone_details_id","lifecycle_milestone_type_id","lifecycle_phase_id"],
    pass_through_cols=["ownership","pairs_with_ownership","has_relpath","has_asst_invp_map",
                        "has_invp_dim","has_invp_bridge","has_invp_version"]
)

# Ownership milestone covering the whole segment (same atomic-pick rationale as lifecycle above)
seg_own_raw = (
    seg_lfe.alias("s")
    .join(
        own.alias("o"),
...
seg_comp = (
    dedupe_segment_rows(
        seg_own_raw, GB_COLS,
        unit_cols=["ownership_milestone_details_id","ownership_milestone_type_id","ownership_phase_id"],
        pass_through_cols=["ownership","pairs_with_ownership","has_relpath","has_asst_invp_map",
                            "has_invp_dim","has_invp_bridge","has_invp_version",
                            "lifecycle_milestone_details_id","lifecycle_milestone_type_id","lifecycle_phase_id"]
    )
...
# Safely store checkpoint data ON persistent cloud storage (can't use localChecpoint if in DABs config the availability is SPOT_WITH_FALLBACK & autoscalilng)
...
# Gaps-and-islands collapse: merge adjacent rows sharing the same natural-grain key and the same
# state (STATE_COLS) into a single row. Uses the shared TemporalToolkit implementation (which
# already merges "opened_by_sources" via union-distinct rather than picking one row's value)
# instead of a local hand-rolled version, since this logic is identical to what other notebooks
# already need.
collapsed = TemporalToolkit.collapse_segments(
    segment_open_events, NATURAL_GRAIN_PARTITION, STATE_COLS, "start", "end"
)
